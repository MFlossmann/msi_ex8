...
for i = 1:N
    ...
    for j = 1:control_dim
        ...
        sigma_osa(i,j) = Phi(i,:)*sigma_theta(:,:,1)*Phi(i,:)';
    end
end
...
x_euler = sim_euler(t, x0, u, param);
x_rk4 = sim_rk4(t, x0, u, param);

%[bar_euler, foo_euler] = pol2cart(x_euler(3,:),...
%                                  ones(1,size(x_euler,2))*deltaT/100);
%[bar_rk4, foo_rk4] = pol2cart(x_rk4(3,:),...
%                              ones(1,size(x_rk4,2))*deltaT/100);
                  
sim_fig = figure;
hold on;
%quiver(x_euler(2,:),x_euler(1,:),...
%       foo_euler, bar_euler, 1, 'b', 'LineWidth', 1.5);
%quiver(x_rk4(2,:), x_rk4(1,:),...
%       foo_rk4, bar_rk4, 1, 'r', 'LineWidth', 1.5);
...
x_euler = sim_euler(t, x0, u, param);
x_rk4 = sim_rk4(t, x0, u, param);
% For the (not implemented) arrows
[bar_euler, foo_euler] = pol2cart(x_euler(3,:),...
